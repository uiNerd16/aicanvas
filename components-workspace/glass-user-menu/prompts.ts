...
## Background
\`<img>\` at \`https://ik.imagekit.io/aitoolkit/bg%20images/Ethereal%20Orange%20Flower%201%20(1).png?updatedAt=1775223702866\`, \`object-cover opacity-60\` over \`bg-sand-950\`. Root: \`relative flex h-full w-full items-center justify-center overflow-hidden bg-sand-950\`. Wrapper: \`relative flex flex-col items-center\` with \`marginTop: -150\`.
...
const USER = { name: 'Jennifer Rivera', email: 'jennifer@studio.io', avatar: 'https://ik.imagekit.io/aitoolkit/Miscellaneous/Avatars/Silhouette%20Profile%20Against%20Gradient%20Background%201.webp' }
const MENU_GROUPS = [
  { label: 'Account',   items: [
...
const ACTIVE_GLOW = '0 8px 40px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.1), 0 0 0 1.5px rgba(255,255,255,0.4), 0 0 20px rgba(255,255,255,0.08)'
\`\`\`

## Phosphor icons
\`import { User, Gear, SignOut, Users, CreditCard, CaretUpDown } from '@phosphor-icons/react'\` — all rendered with \`weight="regular"\`.
...
- CaretUpDown icon: static, no animation wrapper.
- Dropdown panel: \`initial { opacity: 0, scale: 0.95, x: -8, filter: 'blur(4px)' }\`, \`animate { opacity: 1, scale: 1, x: 0, filter: 'blur(0px)' }\`, \`exit { opacity: 0, scale: 0.95, x: -8, filter: 'blur(4px)' }\`, transition \`{ type: 'spring', stiffness: 350, damping: 28 }\`. Wrap with \`<AnimatePresence>\`.
...
Wrapper \`<div ref={ref} className="relative flex flex-col items-center" style={{ marginTop: -150 }}>\`.

Trigger \`motion.button\` \`relative isolate flex cursor-pointer items-center gap-2 rounded-2xl px-4 py-2.5\` style \`{ background: glassPanel.background, border: glassPanel.border }\`. Non-animating blur \`<div className="pointer-events-none absolute inset-0 z-[-1] rounded-2xl" style={glassPanelBlur} />\`. Avatar: \`<img src={USER.avatar} alt={USER.name} className="h-8 w-8 shrink-0 rounded-full object-cover" />\`. Name span \`text-sm font-semibold text-white/80\` text \`{USER.name}\`. \`<CaretUpDown size={16} weight="regular" className="text-white/40" />\` — no motion wrapper.

Dropdown \`<AnimatePresence>\` then \`motion.div\` className \`absolute left-full top-0 ml-2 w-[min(256px,calc(100vw-32px))] rounded-2xl p-2\` style \`{ ...glassPanel, ...glassPanelBlur, transformOrigin: 'top left' }\` — opens to the right, blur applied directly, no child blur div. Initial \`{ opacity: 0, scale: 0.95, x: -8, filter: 'blur(4px)' }\`, animate \`{ opacity: 1, scale: 1, x: 0, filter: 'blur(0px)' }\`, exit reverse. Inside: left-edge highlight \`absolute bottom-6 left-0 top-6 w-[1px]\` background \`linear-gradient(180deg, transparent, rgba(255,255,255,0.18), transparent)\`. Walk a single \`itemIndex\` counter through groups; for each group render \`<p className="mb-0.5 px-3 pt-1 text-[10px] font-semibold uppercase tracking-widest text-white/25">{group.label}</p>\` then map items to \`<MenuItem icon={item.icon} label={item.label} color={item.color} index={itemIndex++} />\`. After all groups: divider \`<div className="mx-2 my-1.5 h-[1px]" style={{ background: 'rgba(255,255,255,0.07)' }} />\` then \`<LogOutItem index={itemIndex} />\`.
...
import { User, Gear, SignOut, Users, CreditCard, CaretUpDown } from '@phosphor-icons/react'
\`\`\`
...
JSX root: \`<div className="relative flex h-full w-full items-center justify-center overflow-hidden bg-sand-950">\`. Background image \`<img src={BACKGROUND} alt="" className="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-60" />\`. Wrapper \`<div ref={ref} className="relative flex flex-col items-center" style={{ marginTop: -150 }}>\`.

Trigger: \`<motion.button onClick={() => setOpen(v => !v)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} animate={{ boxShadow: open ? ACTIVE_GLOW : glassPanel.boxShadow }} transition={{ type: 'spring', stiffness: 300, damping: 26 }} className="relative isolate flex cursor-pointer items-center gap-2 rounded-2xl px-4 py-2.5" style={{ background: glassPanel.background, border: glassPanel.border }}>\`. Inside: blur \`<div className="pointer-events-none absolute inset-0 z-[-1] rounded-2xl" style={glassPanelBlur} />\`; avatar \`<img src={USER.avatar} alt={USER.name} className="h-8 w-8 shrink-0 rounded-full object-cover" />\`; name \`<span className="text-sm font-semibold text-white/80">{USER.name}</span>\`; \`<CaretUpDown size={16} weight="regular" className="text-white/40" />\` — no motion wrapper.

Dropdown: \`<AnimatePresence>{open && (<motion.div initial={{ opacity: 0, scale: 0.95, x: -8, filter: 'blur(4px)' }} animate={{ opacity: 1, scale: 1, x: 0, filter: 'blur(0px)' }} exit={{ opacity: 0, scale: 0.95, x: -8, filter: 'blur(4px)' }} transition={{ type: 'spring', stiffness: 350, damping: 28 }} className="absolute left-full top-0 ml-2 w-[min(256px,calc(100vw-32px))] rounded-2xl p-2" style={{ ...glassPanel, ...glassPanelBlur, transformOrigin: 'top left' }}>...</motion.div>)}</AnimatePresence>\` — opens to the right of the trigger, blur applied directly, no child blur div. Inside dropdown: left-edge highlight \`<div className="absolute bottom-6 left-0 top-6 w-[1px]" style={{ background: 'linear-gradient(180deg, transparent, rgba(255,255,255,0.18), transparent)' }} />\`. For each group: \`<div key={group.label} className="mb-1"><p className="mb-0.5 px-3 pt-1 text-[10px] font-semibold uppercase tracking-widest text-white/25">{group.label}</p>\` then map items \`<MenuItem icon={item.icon} label={item.label} color={item.color} index={itemIndex++} />\`. After all groups: divider \`<div className="mx-2 my-1.5 h-[1px]" style={{ background: 'rgba(255,255,255,0.07)' }} />\` then \`<LogOutItem index={itemIndex} />\`.
...
Background: \`<img>\` at \`https://ik.imagekit.io/aitoolkit/bg%20images/Ethereal%20Orange%20Flower%201%20(1).png?updatedAt=1775223702866\`, \`object-cover opacity-60\`, full-bleed over \`bg-sand-950\`. Center the trigger with a negative top margin (~-150px). The dropdown opens to the right of the trigger.

Trigger: a \`rounded-2xl\` glass pill (\`bg-white/8\`, 1px white border, \`backdrop-blur(24px) saturate(1.8)\`). Inside: a 32px circular \`<img>\` avatar with \`src="https://ik.imagekit.io/aitoolkit/Miscellaneous/Avatars/Silhouette%20Profile%20Against%20Gradient%20Background%201.webp"\`, \`object-cover rounded-full\`, the name "Jennifer Rivera" in semibold white-on-glass, and a static Phosphor \`CaretUpDown\` icon at the end (no rotation animation). The trigger gets a soft white ambient glow (\`box-shadow: 0 0 0 1.5px rgba(255,255,255,0.4), 0 0 20px rgba(255,255,255,0.08)\`) when the menu is open.

Dropdown: a \`rounded-2xl\` frosted glass panel ~256px wide that springs into view **to the right** of the trigger (\`absolute left-full top-0 ml-2\`, \`transformOrigin: 'top left'\`) with a scale + blur entrance (initial scale 0.95, x -8, blur 4px → animate scale 1, x 0, blur 0). It contains two groups with tiny uppercase white/25 group labels:
