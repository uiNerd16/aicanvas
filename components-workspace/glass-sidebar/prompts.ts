...
**Icon tiles** — notification-style tinted badges
- 44×44px tile, rounded-xl (no clipPath, no squircle)
- Icon size 20px, rendered in full accent color via inline style={{ color }}
- Inactive: background \${color}18 (≈9% opacity tint), border 1px solid \${color}22 (≈13% opacity)
- Active: background \${color}28 (≈16% opacity tint), border 1px solid \${color}44 (≈27% opacity)
...
Each of the 7 nav icons lives in its own rounded tile — a notification-style tinted badge with soft, rounded corners. Every icon has its own personality through colour: a calming blue for Home, soft purple for Search, warm amber for Projects, vibrant pink for Notifications, fresh teal for Analytics, earthy gold for Settings, and a warm coral for Profile. The tiles are understated — just a faint wash of the icon's colour as a background with a whisper of a matching border, and the icon drawn in its full accent colour. Clean and modern, no gradients or shine effects.

The active icon has a slightly deeper tint and stronger border — just enough to set it apart without being loud. Hovering any icon feels responsive and springy, like it's physically reacting to your cursor.
...
- GLASS_BLUR_STYLE: {backdropFilter:'blur(24px) saturate(1.8)', WebkitBackdropFilter:'blur(24px) saturate(1.8)'} — on a separate non-animating layer
- GLASS_STYLE: {background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 8px 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.08)'}
...
- Icon tile: 44×44 div, rounded-xl (no clipPath). Notification-style tinted badge: inactive bg color+18 border color+22, active bg color+28 border color+44. CSS transition 0.2s on bg+border. Icon size 20, style={{ color: item.color }} (full accent, not white)
...
- Anchor div width=220 flex items-center justify-start
  - motion.div: style={{width:widthSpring, ...GLASS_STYLE}}, initial x:-20, animate x:0, spring stiffness 200 damping 22 delay 0.1, isolate rounded-3xl overflow-visible
    - Blur layer: absolute inset-0 z-[-1] rounded-3xl, GLASS_BLUR_STYLE (non-animating)
    - 7× NavItemRow
