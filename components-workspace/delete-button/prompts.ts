...
- className: rounded-full bg-red-600 px-6 py-3 text-sm font-semibold tracking-[0.01em] text-white shadow (a soft red drop shadow), outline-none, hover:bg-red-700, focus-visible:ring-4 focus-visible:ring-red-400/45; dark: bg-red-500, hover:bg-red-400, and a lighter-opacity red shadow.
...
- className: flex items-center gap-3 rounded-full border border-red-200 bg-red-100 p-1.5 pr-2 text-red-700 shadow (soft red shadow); dark: border-red-400/20, bg-red-500/15, text-red-300, dimmer shadow.
- Left: an undo motion.button, size-10 (40px) circle, bg-red-600 (dark: bg-red-500), text-white, grid place-items-center, shadow-sm, focus-visible:ring-4 ring-red-400/45, aria-label "Undo account deletion", onClick resets to the default state. whileHover={{ scale: 1.08, rotate: -5 }}, whileTap={{ scale: 0.9 }}, using the shared pill transition. Icon: Phosphor ArrowUUpLeft at size="1.1em", weight="regular", aria-hidden.
- Center: a plain <span> "Cancel Deletion", text-sm font-semibold, whitespace-nowrap, small horizontal padding.
- Right: the countdown badge, a motion.span with the layout prop, size-10 circle, overflow-hidden, bg-red-600 (dark: bg-red-500), text-white, text-sm font-bold, tabular-nums, grid place-items-center, aria-hidden. Inside it, an AnimatePresence initial={false} mode="popLayout" wraps a single motion.span keyed by the current countdown NUMBER, so every digit change triggers an odometer roll: initial={{ y: '100%', opacity: 0 }}, animate={{ y: '0%', opacity: 1 }}, exit={{ y: '-100%', opacity: 0 }}, transition={{ duration: 0.24, ease: [0.22, 1, 0.36, 1] }}. This makes each new digit slide up from below while the old one slides out the top, clipped by the circle's overflow-hidden.

PILL 3, DELETED ("Account Deleted")
- A motion.div, role="status", aria-live="polite".
- className: flex items-center gap-2 rounded-full border border-stone-300 bg-stone-200 px-5 py-3 text-sm font-semibold text-stone-600 shadow-sm; dark: border-stone-700, bg-stone-800, text-stone-300. This is a deliberately muted/desaturated palette compared to the reds; it reads as "resolved", not alarming.
- A small check badge before the label: size-5 circle, bg-stone-500 (dark: bg-stone-600), text-white, grid place-items-center. Icon: Phosphor Check at size="0.9em", weight="regular", aria-hidden.
