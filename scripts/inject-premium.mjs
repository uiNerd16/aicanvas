...
const V2_FALLBACK_NAMES = ['MetricChart', 'Gauge', 'Waveform', 'MediaCard', 'DataTable', 'MusicPlayer', 'FunnelChart', 'Orb', 'Nodes', 'Burst', 'GridBackdrop', 'VoidBackdrop', 'HorizonBackdrop', 'ContourBackdrop']
...
// Degraded fallback for template compositions — same contract as
// V2_FALLBACK_NAMES: tracked routes (templates/sign-in/page.tsx) import these
// vault-authored example folders by relative path, so a build without the
// vault still needs a module there. No-op when injection (or a dev checkout)
// already materialised the folder; sentinel-marked so nothing registers it.
// Runs on BOTH the full path and the free-only skip path.
const EXAMPLE_FALLBACKS = { andromeda: ['sign-in'] }
function writeExampleFallbacks() {
  const written = []
  for (const [system, slugs] of Object.entries(EXAMPLE_FALLBACKS)) {
    for (const slug of slugs) {
      const relDir = `design-systems/${system}/examples/${slug}`
      const outFile = join(ROOT, relDir, 'index.tsx')
      if (existsSync(outFile)) continue
      const name = slug.replace(/(^|-)([a-z0-9])/g, (_, __, c) => c.toUpperCase()) + 'Placeholder'
      mkdirSync(dirname(outFile), { recursive: true })
      writeFileSync(outFile, [
        FREE_DS_PLACEHOLDER_SENTINEL,
        '// AUTO-GENERATED by scripts/inject-premium.mjs on a degraded/partial build —',
        '// the vault-authored template composition was not injected. Never committed.',
        '// @ts-nocheck',
        '',
        ...placeholderFn(name),
        '',
...
if (!source) {
  ...
  // Clean BEFORE writing example fallbacks: a stale real example from a prior
  // full inject must be removed so the placeholder actually lands.
  cleanupFreeDs(freePlaceholders)
  freePlaceholders.push(...writeExampleFallbacks())
...
examplePaths.push(...writeExampleFallbacks())
