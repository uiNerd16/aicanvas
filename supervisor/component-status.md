...
| ripple-type | built | Kinetic typography — "HELLO"/"RIPPLE" SVG text with feTurbulence + feDisplacementMap filter. scale/baseFrequency mutated on refs each `useAnimationFrame` tick so letters ripple like liquid. REST_SCALE 16 / HOVER_SCALE 34, REST_FREQ 0.014 / HOVER_FREQ 0.009, 0.22 Hz breathing. SVG viewBox 800×520, font-size 220, weight 900. Unique filter id via useId. dualTheme (#0A0A0A/#EFEEE6 swap), radial vignette. Reduced-motion holds 0.6× base scale static. Registry entry appended, codegen ran, tsc clean. |
