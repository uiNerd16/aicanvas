...
export function generateTSX(config: Config, svgSource: string): string {
  ...
  const svgB64 = toBase64(svgSource)

  return `'use client'

// npm install three @react-three/fiber
//
// Generated by AI Canvas Studio — 60K Particles
// https://aicanvas.me/studio/60k-particles
...
function Field() {
  ...
  useEffect(() => {
    let alive = true
    sample(DENSITY).then((d) => { if (alive) setData(d) })
    return () => { alive = false }
  }, [])
