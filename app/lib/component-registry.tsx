...
const GeistPixelGrid = localFont({ src: './fonts/GeistPixel-Grid.woff2' })
      ...
      >
        {rows.map((chars, rowIdx) => (
          <div key={rowIdx} className={\`flex items-center gap-1 \${GeistPixelGrid.className}\`}>
            ...
            {chars.map((ch, i) => (
              <span key={i} className="text-6xl leading-none tracking-widest" style={{ color: '#BECF5D' }}>
                {ch.display}
              </span>
            ))}
